...
from app.assistant import AIVoiceAssistant
from app.tts import text_to_speech
from datetime import datetime
from  app.config.config import LANGUAGE_NAMES,SYSTEM_PROMPT
...
async def handle_client(websocket, path):
    ...
    client_conversation = [{
        ...
        "content": SYSTEM_PROMPT.format(
            ...
            language=LANGUAGE_NAMES.get(CURRENT_LANGUAGE, "English")
        )
    }]
            ...
            try:
                ...
                # ── TEXT MESSAGE ─────────────────────────────────────────
                    ...
                    client_conversation[0] = {
                        ...
                        "content": SYSTEM_PROMPT.format(
                            datetime=datetime.now().strftime("%A, %B %d %Y %I:%M %p"),
                            language=LANGUAGE_NAMES.get(new_lang, "English")
                        )
                    }
                    print(f"[LANG] Switched to: {LANGUAGE_NAMES.get(new_lang, new_lang)}")
                    await websocket.send(json.dumps({
                        "type": "language_switched",
                        "language": new_lang,
                        "name": LANGUAGE_NAMES.get(new_lang, new_lang)
                    }))
                ...
                # ── HUMAN-IN-THE-LOOP DECISION ────────────────────────────
                    ...
                    if len(client_conversation) > 7:
                        trimmed = [client_conversation[0]] + client_conversation[-6:]
...
async def _init_rag():
    """Initialize RAG tools in THIS WebSocket process before accepting connections."""
    import os, yaml
    from app.settings import MODEL_CONFIG_PATH, EMBD_MODEL_DIR, VECTOR_STORE_PATH
    ...
    SCHOOL_YAML_PATH = os.path.join(
        os.path.dirname(__file__), "app/config/school_vector_indexes.yaml"
    )
    ...
    print("[RAG] Initializing RAG tools...")
    # init_rag_tools(
    #     llm                 = llm,
    ...
    # )
    print("[RAG] ✅ RAG tools ready in WebSocket process.")
...
async def start_server():
    ...
    async with websockets.serve(
            ...
            ping_timeout=120,  # wait 120s for pong before closing
    ):
