...
from app.config.config import SYSTEM_PROMPT
...
GROQ_LLM            = os.getenv("GROQ_LLM", "true").lower() == "true"
GROQ_STT            = os.getenv("GROQ_STT",  "true").lower() == "true"
...
LOCAL_WHISPER_MODEL = os.getenv(
    "LOCAL_WHISPER_MODEL",
    r"/Personal-Voice-Agent/faster-whisper-mediun-int8"
)
...
if GROQ_STT:
    ...
    print(f"[STT] Loading faster-whisper from: {LOCAL_WHISPER_MODEL}")
...
# ══════════════════════════════════════════════════════════════
# TOOL IMPORT (lazy, cached)
# ══════════════════════════════════════════════════════════════


def _get_tools():
    from app.tools.registry import ALL_TOOLS, FUNCTION_MAP   # ← re-import every call
    return ALL_TOOLS, FUNCTION_MAP



AUTO_APPROVE_TOOLS = {
    "get_current_time", "get_current_date", "get_reminders",
    "set_reminder", "delete_reminder", "send_message",
    "get_messages", "searxng_search",
}
...
_RESULT_KEYWORDS = re.compile(
    r'\b(result|grade|marks|score|passed|failed)\b'   # ← removed "exam"
    r'(?!.*\b(timetable|schedule|date|when|time)\b)', re.I
)
...
_SEARCH_KEYWORDS = re.compile(
    r'\b(latest news|recent news|news about|search for|breaking news|'
    r'current news|what happened|happening in|who won|cricket score|'
    r'football score|stock price|weather in|price of)\b', re.I
)
_REFUSAL_PHRASES = re.compile(
    r'(cannot provide|cannot assist|cannot answer|cannot explain|'
    r"can't explain|can't provide|don't have access|do not have access|"
    r'not able to provide|limited to|my capabilities|my current capabilities|'
    r'beyond my|outside my|i am not able)', re.I
)
_QUESTION_START = re.compile(
    r'^(what|how|why|who|when|where|which|explain|tell me about|describe)\b', re.I
)
...
_WRONG_FOR_RESULT = {
    "get_current_time", "get_current_date", "get_reminders",
    "send_message", "get_messages", "searxng_search",
}

_WRONG_FOR_SEARCH = {
    "get_current_time", "get_current_date",
    "send_message", "get_messages", "get_exam_result",
}

_CASUAL_PHRASES = re.compile(
    r'^(hi|hello|hey|how are you|what are you doing|who are you|'
    r'what is your name|thank you|thanks|ok|okay|bye|goodbye)\b', re.I
)
...
PARAM_SCHEMA = {
    "class_name": {
        "description": "class or grade number between 1 and 12",
        "examples":    ["3", "4", "10", "12"],
        "invalid":     ["English", "Math", "unknown", "second", "next"],
        "validate":    lambda v: re.match(r'^\d{1,2}$', str(v).strip()) and 1 <= int(str(v).strip()) <= 12,
        "clean":       lambda v: str(v).strip().rstrip(".").strip(),
        "regex":       re.compile(
            r'\b(?:class|std|standard|grade|klass)?\s*'
            r'(1[0-2]|[1-9])\b',
            re.I
        ),
    },
    "roll_number": {
        "description": "student roll number, may be numeric or alphanumeric",
        "examples":    ["12", "234330", "STU004", "E665655"],
        "invalid":     ["unknown", "null", "none"],
        "validate":    lambda v: bool(re.match(r'^[A-Za-z0-9]{1,15}$', str(v).strip())),
        "clean":       lambda v: re.sub(r'[\s\-,]+', '', str(v)).strip(),
        "regex":       re.compile(r'\b([A-Za-z]{0,3}\d{3,8})\b|\b(\d{1,6})\b'),
    },
    "section": {
        "description": "class section letter A through E",
        "examples":    ["A", "B", "C"],
        "invalid":     [],
        "validate":    lambda v: bool(re.match(r'^[A-Ea-e]$', str(v).strip())),
        "clean":       lambda v: str(v).strip().upper(),
        "regex":       re.compile(r'\b(?:section|sec)?\s*([A-Ea-e])\b'),
    },
}
        ...
        try:
            ...
            if GROQ_STT:
                ...
                segments_gen, info = _whisper.transcribe(
                    tmp_path, language=language, beam_size=3, best_of=3,
                    vad_filter=True,
