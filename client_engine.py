...
import pygame
import webrtcvad
from reasoning_engine.tts.tts import text_to_speech, switch_tts_language
import uuid
import yaml
...
# Load the YAML file
with open("configs/client_config.yaml", "r", encoding="utf-8") as f:
...
DETECTION_THRESHOLD  = client_config['detection_threshold']
ENABLE_SPEEX         = client_config['enable_speex']
VAD_THRESHOLD        = client_config['vad_threshold']
SMOOTHING_WINDOW     = client_config['smoothing_window']
COOLDOWN_TIME        = client_config['cooldown_time']
INSTANT_SCORE_GATE   = client_config['instant_score_gate']

# Audio
CHUNK_SIZE = int(client_config['chunk_size'])
FORMAT     = pyaudio.paInt16
CHANNELS   = int(client_config['channels'])
RATE       = int(client_config['rate'])

# Sound file paths
RESPONSE_AUDIO_PATH = client_config['response_audio_path']
HITL_CONFIRM_SOUND  = client_config['hitl_confirm_sound']
HITL_REVIEW_SOUND   = client_config['hitl_review_sound']
INSTRUCTION_SOUND   = client_config['instruction_sound']
WAIT_MOMENT_SOUND   = client_config['wait_moment_sound']
THINKING_SOUND      = client_config['thinking_sound']

# WebRTC VAD
VAD_MODE           = client_config['vad_mode']
VAD_FRAME_DURATION = client_config['vad_frame_duration']
VAD_FRAME_SIZE     = int(RATE * VAD_FRAME_DURATION / 1000)

# Recording
RECORDING_DURATION = client_config['recording_duration']
MIN_RECORDING_TIME = client_config['min_recording_time']
SILENCE_DURATION   = client_config['silence_duration']
VAD_SILENCE_CHUNKS = int(SILENCE_DURATION * 1000 / VAD_FRAME_DURATION)

# Validation
MIN_SPEECH_FRAMES_REQUIRED = client_config['min_speech_frames_required']
MIN_AUDIO_AMPLITUDE        = client_config['min_audio_amplitude']
MIN_SPEECH_PERCENTAGE      = client_config['min_speech_percentage']

# WebSocket
WEBSOCKET_URI = client_config['websocket_uri']

# HITL
HITL_ENABLED = client_config['hitl_enabled']
...
def switch_language(lang_code: str):
    assist_config['CURRENT_LANGUAGE'] = lang_code
